...
Else
    ...
    Else
        ...
        sHtml = "<p><select id='file_name' style='width: 100%'>" & GetFileSelect(sFolderPath)  & "</select></p>" & _
                "<p align=center><input type='button' value='Convert PDF to Excel' onclick='Send()'> " & _
                                "<input type='button' value='Close' onclick='self.Close()'></p>"

        Set oRet = OpenDialog(sHtml, "file_name", 300, 200, "Convert PDF to Excel")
...
If sFilePath = "" Then
    MsgBox "Please drag and drop a PDF file or folder on top this file to convert it to Excel."
    WScript.Quit 
End If
...
Sub ProcessFile (sFilePath)
    ...
    Dim sOutPath: sOutPath = sFolderPath & ".xls"        
    If fso.FileExists(sOutPath) = False Then
        BreakePdfToPages sFilePath
        PdfToExcel sFilePath
        MergeHtmlFiles sFilePath 
    End If
...
Sub MergeHtmlFiles (sFilePath)
    ...
    For Each oFile In oFolder.Files
        If fso.GetExtensionName(oFile.Name) = "htm" Then
...
Sub PdfToExcel(sFilePath)
    ...
    For Each oFile In oFolder.Files
        If fso.GetExtensionName(oFile.Name) = "pdf" Then
            ...
            sTextFilePath = sFolderPath & "\" & fso.GetBaseName(oFile.Path) & ".htm"
...
Function SendFile(sPrompt, sFilePath)
    ...
    SendFile = ie.ExecuteScript("return document.querySelector(""div[data-testid='markdown-reply']"").innerHTML")
End Function
...
Sub CreateHtaFile(sHtaFilePath, sHtml, sFields, iWidth, iHeight, sTitle)
    ...
    f.WriteLine "<html><title>Convert PDF to Excel</title><head><HTA:APPLICATION ID=oHTA SINGLEINSTANCE=""yes"" SCROLL=""no""/></head>"
